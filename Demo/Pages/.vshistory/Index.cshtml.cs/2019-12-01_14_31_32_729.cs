        ...
        public async Task<IActionResult> OnGetAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var node = pageDocument.DocumentNode.SelectSingleNode("//body");

                    var web = new HtmlWeb();
                    HtmlDocument document = await web.LoadFromWebAsync(pageURL);
                    var red = document.DocumentNode.SelectSingleNode("//head/title");
                    Console.WriteLine("Node Name: " + red.Name + "\n" + red.OuterHtml);
                    HtmlDocument htmlPage = document.Load(pageURL);
